...
if [ ! -f wp-config.php ]; then
    wp config create --dbname=$(cat $DB_NAME_FILE) --dbuser=$(cat $DB_USER_FILE) --dbpass=$(cat $DB_PASSWORD_FILE) --dbhost=$(cat $DB_HOST_FILE) --dbprefix=$(cat $DB_PREFIX_FILE) --allow-root
fi
